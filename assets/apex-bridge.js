/** ApexFreePort bridge — category-filtered catalog + up to 6 photos */
(function (global) {
  ...
  function productMedia(i) {
    ...
    if (Array.isArray(i.images)) {
      i.images.forEach(function (p) { add(p, "img"); });
    }
  ...
  function mediaBlock(parts, idx) {
    ...
    return (
      '<div class="apex-carousel relative h-52 rounded-xl overflow-hidden mb-4 border border-emerald-900/40 bg-zinc-950" data-car="' + idx + '">' +
      slides +
      (parts.length > 1
  ...
  function productCard(i, idx, compact) {
    ...
    var btn = disabled
      ...
      : '<button type="button" onclick="addToCart(\'' + nameSafe + "','" + i.sku + "'," + (Number(i.price) || 0) +
  ...
  async function renderCatalog(selector, category) {
    ...
    try {
      ...
      if (!items.length) {
        ...
        if (status) {
          status.textContent = "Live catalog · " + (category || "all") + " · empty";
      ...
      el.innerHTML = '<p class="text-amber-400/90 text-center col-span-full py-12">Inventory bridge offline. Check ApexFreePort feed for this store.</p>';
      if (status) {
