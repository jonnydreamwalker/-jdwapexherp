 ...
 * Category on the page must match FreePort item.category (e.g. "Live Plants").
 */
(function () {
  ...
  function catMatch(itemCat, want) {
    ...
    if (!b) return true;
    if (a === b) return true;
    var plantAliases = ["live plants", "live plant", "plants", "plant"];
    if (plantAliases.indexOf(b) >= 0 && plantAliases.indexOf(a) >= 0) return true;
    return false;
  }
  ...
  async function loadCatalog() {
    ...
    if (!el) {
      console.warn("[Apex] #apex-catalog missing");
      return;
    }
    var cat = (el.getAttribute("data-category") || "").trim();
    if (!cat) {
      console.warn("[Apex] data-category missing on #apex-catalog");
      return;
    }
    if (status) {
    ...
    try {
      ...
      if (!items.length) {
        var res2 = await fetch(API + "/api/products?store=herp", { mode: "cors", cache: "no-store", credentials: "omit" });
        if (res2.ok) {
      ...
      if (!items.length) {
        el.innerHTML =
          '<p class="text-zinc-500 text-center col-span-full py-12">' +
          "No products in <span class=\"text-emerald-400\">" + esc(cat) + "</span> yet. " +
          "Add them in ApexFreePort with this exact category name.</p>";
        if (status) {
