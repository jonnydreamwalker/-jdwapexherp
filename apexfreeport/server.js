...
const STORE_META = {
  herp: {
    name: "Apex Herp",
    defaultCategories: ["Hardscape", "Lighting", "Substrates", "Nutrition", "Apparel", "Enclosures", "Heating", "Hardware", "Deals"],
  },
  k9: { name: "Apex K9", defaultCategories: ["Food", "Gear", "Training", "Beds", "Health", "Apparel", "Deals"] },
  feline: { name: "Apex Feline", defaultCategories: ["Food", "Litter", "Play", "Beds", "Health", "Apparel", "Deals"] },
};
...
function normalizeImages(item) {
  var imgs = [];
  if (Array.isArray(item.images)) item.images.forEach(function (p) { if (p && imgs.indexOf(p) < 0) imgs.push(p); });
...
function normalizeVideos(item) {
  var vids = [];
  if (Array.isArray(item.videos)) item.videos.forEach(function (p) { if (p && vids.indexOf(p) < 0) vids.push(p); });
  if (item.video && vids.indexOf(item.video) < 0) vids.unshift(item.video);
  vids = vids.slice(0, 2);
  item.videos = vids;
  item.video = vids[0] || "";
  return vids;
...
function migrate(d) {
  ...
  STORE_IDS.forEach(function (id) {
    if (!d.stores[id]) d.stores[id] = { id: id, name: STORE_META[id].name, publicFeed: false, items: [] };
    ...
    d.stores[id].items.forEach(function (it) {
      normalizeImages(it);
      normalizeVideos(it);
    });
  });
...
function read() { return migrate(JSON.parse(fs.readFileSync(DATA, "utf8"))); }
function write(d) { d.updated = new Date().toISOString(); fs.writeFileSync(DATA, JSON.stringify(d, null, 2)); }
function storeOf(d, id) { return d.stores[normalizeStoreId(id)]; }
function findItemIndex(items, sku) {
  sku = String(sku || "").trim();
  for (var i = 0; i < items.length; i++) if (String(items[i].sku).trim() === sku) return i;
  return -1;
}
...
function publicItem(i) {
  ...
  return {
    ...
    lane: i.lane || "direct", status: i.status || "active",
    image: imgs[0] || "", images: imgs, video: vids[0] || "", videos: vids,
    location: i.location || "",
  };
}
function feedOff(res, storeId) {
  return res.status(503).json({ error: "public_feed_off", message: "Website inventory feed is OFF for this store.", store: storeId, publicFeed: false, items: [] });
}
function saveImageDataUrl(storeId, sku, dataUrl) {
  var m = String(dataUrl).match(/^data:image\/(png|jpeg|jpg|webp);base64,(.+)$/i);
  if (!m) return null;
...
function saveVideoDataUrl(storeId, sku, dataUrl) {
  var m = String(dataUrl).match(/^data:video\/(mp4|webm|quicktime);base64,(.+)$/i);
  if (!m) return null;
  var ext = m[1].toLowerCase() === "quicktime" ? "mov" : m[1].toLowerCase();
  if (ext !== "mp4" && ext !== "webm") ext = "mp4";
  var safe = String(sku).replace(/[^a-zA-Z0-9_-]/g, "_");
  var fname = storeId + "-" + safe + "-vid-" + Date.now() + "-" + Math.floor(Math.random() * 9999) + "." + ext;
  var buf = Buffer.from(m[2], "base64");
  if (buf.length > 12000000) return null;
  fs.writeFileSync(path.join(UPLOADS, fname), buf);
  return "/uploads/" + fname;
}

app.get("/health", function (req, res) {
  var d; try { d = read(); } catch (e) { d = { stores: {} }; }
  var feeds = {};
  STORE_IDS.forEach(function (id) { feeds[id] = !!(d.stores && d.stores[id] && d.stores[id].publicFeed); });
  res.json({ ok: true, service: "ApexFreePort", multiStore: true, stores: feeds,
...
app.get("/api/stock", function (req, res) {
  try {
    ...
    var d = read(); var st = storeOf(d, storeId);
    if (!st.publicFeed) return feedOff(res, storeId);
    res.json({ store: storeId, storeName: st.name, warehouse: d.warehouse, updated: d.updated, publicFeed: true, items: (st.items || []).map(publicItem) });
...
app.get("/api/products", function (req, res) {
  try {
    ...
    res.json({ store: storeId, storeName: st.name, warehouse: d.warehouse, updated: d.updated, publicFeed: true, items: items });
  } catch (e) { res.status(500).json({ error: "fail" }); }
});
...
app.get("/api/inventory", auth, function (req, res) {
  ...
  res.json({
    warehouse: d.warehouse, updated: d.updated, store: storeId, storeName: st.name, publicFeed: !!st.publicFeed, categories: cats,
    items: (st.items || []).map(function (it) { normalizeImages(it); normalizeVideos(it); return it; }),
...
app.post("/api/inventory/adjust", auth, function (req, res) {
  ...
  st.items[idx].qty = Math.max(0, (st.items[idx].qty || 0) + Number(req.body.delta || 0));
  write(d);
  res.json({ ok: true, item: publicItem(st.items[idx]) });
});
...
app.post("/api/inventory/reorder", auth, function (req, res) {
  ...
  var tmp = st.items[idx]; st.items[idx] = st.items[j]; st.items[j] = tmp;
  write(d);
...
app.post("/api/inventory/category", auth, function (req, res) {
  ...
  if (!st.categories.some(function (c) { return String(c).toLowerCase() === name.toLowerCase(); })) st.categories.push(name);
  write(d);
...
app.post("/api/inventory/item", auth, function (req, res) {
  ...
  var prevVids = normalizeVideos(prev);
  var row = {
    sku: newSku, name: String(b.name).trim(), category: cat,
    ...
    reserved: Number(prev.reserved) || 0, reorder: Number(prev.reorder) || 0,
    lane: b.lane === "external" ? "external" : b.lane === "direct" ? "direct" : prev.lane || "direct",
    status: b.status || prev.status || "active",
    image: prev.image || "", images: prevImgs.slice(),
    video: prev.video || "", videos: prevVids.slice(),
    location: b.location || prev.location || "WH-A1", unit: prev.unit || "each",
  };
  ...
  if (b.video != null && b.video !== "") {
    row.video = b.video;
    if (row.videos.indexOf(b.video) < 0) row.videos.unshift(b.video);
  }
  normalizeImages(row); normalizeVideos(row);
  if (idx >= 0) { st.items[idx] = Object.assign({}, prev, row); row = st.items[idx]; }
...
app.post("/api/inventory/image", auth, function (req, res) {
  try {
    ...
    var storeId = normalizeStoreId(b.store);
    var url = saveImageDataUrl(storeId, b.sku, b.dataUrl);
    ...
    item.images.push(url); item.image = item.images[0];
    write(d);
...
app.post("/api/inventory/images", auth, function (req, res) {
  try {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      var url = saveImageDataUrl(storeId, b.sku, list[i]);
      if (url) { item.images.push(url); added.push(url); }
    }
...
app.post("/api/inventory/videos", auth, function (req, res) {
  try {
    ...
    var list = Array.isArray(b.dataUrls) ? b.dataUrls : b.dataUrl ? [b.dataUrl] : [];
    for (var i = 0; i < list.length; i++) {
      if (item.videos.length >= 2) break;
      var url = saveVideoDataUrl(storeId, b.sku, list[i]);
      if (url) { item.videos.push(url); added.push(url); }
    }
    if (Array.isArray(b.urls)) {
      b.urls.forEach(function (u) {
        u = String(u || "").trim();
        if (u && item.videos.length < 2 && item.videos.indexOf(u) < 0) {
          item.videos.push(u);
          added.push(u);
        }
      });
    ...
    write(d);
    res.json({ ok: true, added: added, videos: item.videos });
  } catch (e) { res.status(500).json({ error: "upload failed" }); }
});

app.post("/api/webhook/square", function (req, res) { res.json({ ok: true }); });
app.post("/api/webhook/stripe", function (req, res) { res.json({ ok: true }); });
app.post("/api/webhook/paypal", function (req, res) { res.sendStatus(200); });
app.get("/", function (req, res) { res.redirect("/admin"); });
app.listen(PORT, function () { console.log("ApexFreePort multi-store on " + PORT); });
