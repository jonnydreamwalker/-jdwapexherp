...
FileStore.prototype.get = function (sid, cb) {
  try {
    ...
    return cb(null, JSON.parse(fs.readFileSync(f, "utf8")));
  } catch (e) {
    return cb(null, null);
  }
};
FileStore.prototype.set = function (sid, sess, cb) {
  try {
    const f = path.join(SESSIONS, String(sid).replace(/[^a-zA-Z0-9_-]/g, "") + ".json");
...
app.use(function (req, res, next) {
  ...
  res.setHeader("Access-Control-Allow-Headers", "Content-Type,x-apex-secret,x-dealer-token");
...
function persistDataUrl(sku, dataUrl, idx) {
  ...
  var ext = "bin";
  if (mime.indexOf("jpeg") >= 0 || mime.indexOf("jpg") >= 0) ext = "jpg";
  else if (mime.indexOf("png") >= 0) ext = "png";
  else if (mime.indexOf("webp") >= 0) ext = "webp";
  else if (mime.indexOf("gif") >= 0) ext = "gif";
  var safe = String(sku || "img").replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 80);
  var name = safe + (idx ? "_" + idx : "") + "." + ext;
  try {
...
function hydrateItemPhotos(item) {
  if (!item) return item;
  var changed = false;
  if (item.image && String(item.image).indexOf("data:") === 0) {
    item.image = persistDataUrl(item.sku, item.image, 0);
    changed = true;
  }
  if (Array.isArray(item.images)) {
    item.images = item.images.map(function (u, i) {
      if (typeof u === "string" && u.indexOf("data:") === 0) {
        changed = true;
        return persistDataUrl(item.sku, u, i);
      }
      return u;
    });
    if (item.images[0] && (!item.image || String(item.image).indexOf("data:") === 0)) {
      item.image = item.images[0];
    }
  }
  if (Array.isArray(item.videos)) {
    item.videos = item.videos.map(function (u, i) {
      if (typeof u === "string" && u.indexOf("data:") === 0) {
        changed = true;
        return persistDataUrl(item.sku + "_vid", u, i);
      }
      return u;
    });
  }
...
function defaultData() {
  return {
    ...
    stores: {
      herp: { name: "Apex Herp", publicFeed: false, categories: ["Hardscape", "Lighting", "Substrates", "Nutrition", "Apparel"], items: [] },
      k9: { name: "Apex K9", publicFeed: false, categories: ["Gear", "Training", "Apparel"], items: [] },
      feline: { name: "Apex Feline", publicFeed: false, categories: ["Gear", "Enrichment", "Apparel"], items: [] }
    },
    movements: []
  };
}
...
function applyPhotos(item, b) {
  if (b.image != null) {
    ...
    if (img.indexOf("data:") === 0) img = persistDataUrl(item.sku, img, 0);
    item.image = img;
    if (item.image) {
      item.images =
        Array.isArray(b.images) && b.images.length
          ? b.images.slice(0, 10).map(function (u, i) {
              return typeof u === "string" && u.indexOf("data:") === 0 ? persistDataUrl(item.sku, u, i) : u;
            })
          : [item.image];
    } else {
      item.images = Array.isArray(b.images)
        ? b.images.slice(0, 10).map(function (u, i) {
            return typeof u === "string" && u.indexOf("data:") === 0 ? persistDataUrl(item.sku, u, i) : u;
          })
        : [];
    ...
    item.images = b.images.slice(0, 10).map(function (u, i) {
      return typeof u === "string" && u.indexOf("data:") === 0 ? persistDataUrl(item.sku, u, i) : u;
    });
    item.image = item.images[0] || "";
  }
  ...
  const items = (o.items || []).map(function (it) {
    ...
    return Object.assign({}, it, {
      ...
      shippingTerms:
        it.shippingTerms ||
        inv.shippingTerms ||
        (drop ? "Shipping on supplier terms — not calculated from FL warehouse" : "FL warehouse / direct")
    });
  });
...
function isPublicListed(i) {
  ...
  // Bulk / dealer only: SKU ending in -B stays on wholesale portal, never public store
  var sku = String(i.sku || "").toUpperCase();
  var segs = sku.split("-").filter(Boolean);
  if (segs.length && segs[segs.length - 1] === "B") return false;
  var pool = String(i.pool || i.channel || "").toLowerCase();
  if (pool === "wholesale" || pool === "dealer") return false;
  if (i.dealerOnly === true || i.wholesaleOnly === true) return false;
  if (i.showOnSite === false || i.showOnSite === "false") return false;
  return true;
}
...
app.get("/admin/app.js", auth, function (req, res) {
  res.type("application/javascript").sendFile(path.join(__dirname, "admin", "app.js"));
});
...
app.get("/api/inventory", auth, function (req, res) {
  ...
  let migrated = false;
  (st.items || []).forEach(function (it) {
    if (hydrateItemPhotos(it)) migrated = true;
  });
  if (migrated) {
    try { write(d); } catch (e) { console.error("migrate write", e.message); }
  }
...
app.post("/api/inventory/item", auth, function (req, res) {
  ...
  let item = (st.items || []).find(function (i) { return i.sku === original || i.sku === sku; });
  if (!item) {
    item = {
      sku: sku, name: b.name || sku, category: b.category || "", description: b.description || "",
      price: Number(b.price) || 0, qty: Number(b.qty) || 0, reserved: Number(b.reserved) || 0,
      lane: lane, dropShip: dropShip, supplier: b.supplier || "",
      shippingTerms: b.shippingTerms || (dropShip ? "Supplier ships — rate on their terms" : ""),
      status: b.status || "active", location: b.location || "", listed: true,
      image: "", images: [], videos: []
    };
    st.items.push(item);
  } else {
...
app.post("/api/inventory/adjust", auth, function (req, res) {
  ...
  write(d);
  res.json({ ok: true, item: { sku: item.sku, qty: item.qty, name: item.name, price: item.price } });
});
...
app.post("/api/inventory/images", auth, function (req, res) {
  ...
  urls.slice(0, 10).forEach(function (u, i) {
    if (typeof u === "string" && u.indexOf("data:") === 0) {
      item.images.push(persistDataUrl(item.sku, u, item.images.length + i));
...
app.post("/api/inventory/videos", auth, function (req, res) {
  ...
  const urls = Array.isArray(b.dataUrls) ? b.dataUrls : [];
  urls.slice(0, 2).forEach(function (u, i) {
    if (typeof u === "string" && u.indexOf("data:") === 0) {
      item.videos.push(persistDataUrl(item.sku + "_vid", u, item.videos.length + i));
    } else if (typeof u === "string") {
      item.videos.push(u);
    }
  });
...
app.get("/api/fulfillment/orders", auth, function (req, res) {
  try {
    const d = readOrders();
    const inv = skuLookup();
    const orders = sortOrders(d.orders || []).map(function (o) { return enrichOrder(o, inv); });
...
app.listen(PORT, function () {
  ...
  console.log("Photos: disk /uploads (kept)");
  console.log("Sessions: file store (survive restart)");
});
