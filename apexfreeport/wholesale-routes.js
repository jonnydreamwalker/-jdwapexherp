...
module.exports = function mountWholesale(app, opts) {
  ...
  function categoryMatch(itemCat, want) {
    var a = String(itemCat || "").toLowerCase().trim();
    var b = String(want || "").toLowerCase().trim();
    if (!b) return true;
    if (a === b) return true;
    if (a.indexOf("plant") >= 0 && b.indexOf("plant") >= 0) return true;
    return false;
  }
  ...
  app.patch("/api/wholesale/dealers/:id", auth, function (req, res) {
    try {
      const id = String(req.params.id || "");
      const b = req.body || {};
      const db = readDealers();
      const d = (db.dealers || []).find(function (x) { return x.id === id; });
      if (!d) return res.status(404).json({ error: "Dealer not found" });
      if (b.business_name != null) d.business_name = String(b.business_name).trim();
      if (b.contact_name != null) d.contact_name = String(b.contact_name).trim();
      if (b.email != null) d.email = String(b.email).trim().toLowerCase();
      if (b.phone != null) d.phone = String(b.phone).trim();
      if (b.website != null) d.website = String(b.website).trim();
      if (b.status != null) {
        var st = String(b.status).trim().toLowerCase();
        var ok = { active: 1, pending_setup: 1, suspended: 1, disabled: 1 };
        if (!ok[st]) return res.status(400).json({ error: "status must be active, pending_setup, suspended, or disabled" });
        if (isOwnerDealer(d) && st !== "active") {
          return res.status(400).json({ error: "Owner dealer must stay active" });
        }
        d.status = st;
        if (st !== "active") d.sessionToken = null;
      }
  ...
  app.delete("/api/wholesale/dealers/:id", auth, function (req, res) {
    try {
      const id = String(req.params.id || "");
      const db = readDealers();
      const d = (db.dealers || []).find(function (x) { return x.id === id; });
      if (!d) return res.status(404).json({ error: "Dealer not found" });
      if (isOwnerDealer(d) || d.id === "DLR-OWNER") {
        return res.status(400).json({ error: "Cannot delete the owner dealer account" });
      }
      db.dealers = (db.dealers || []).filter(function (x) { return x.id !== id; });
      writeDealers(db);
  ...
  app.get("/api/wholesale/catalog", function (req, res) {
    ...
    try {
      ...
      if (cat) {
        ...
        items = items.filter(function (i) {
          return categoryMatch(i.category, c);
        });
      }
