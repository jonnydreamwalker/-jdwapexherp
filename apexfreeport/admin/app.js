(function () {
  ...
  function isWholesaleItem(it) {
    if (!it) return false;
    var w = Number(it.weightLb) || 0;
    if (w >= 50) return true;
    if (it.dealerEligible === true && w >= 50) return true;
    var n = String(it.name || "").toLowerCase();
    var m = n.match(/(\d+)\s*lb/);
  ...
  function openEdit(sku) {
    ...
    if ($("p_wholesale")) {
      var pool = String(it.pool || it.channel || "").toLowerCase();
  ...
  function saveProduct() {
    ...
    var body = {
      ...
      pool: ($("p_wholesale") && $("p_wholesale").checked) ? "wholesale" : "retail",
      dealerEligible:!!($("p_wholesale") && $("p_wholesale").checked),
  ...
  function saveOrder() {
    ...
    api("/api/inventory/sort", { method: "POST", body: { store: store, skus: skus } })
      .then(function () {
        ...
        var map = {};
        items.forEach(function (it) { map[it.sku] = it; });
        var ordered = [];
        skus.forEach(function (s) { if (map[s]) ordered.push(map[s]); });
        items.forEach(function (it) { if (skus.indexOf(it.sku) < 0) ordered.push(it); });
        items = ordered;
      })
  ...
  function renderTable() {
    ...
    var cols = wholesaleMode ? 9 : 7;
    if (!list.length) {
      tb.innerHTML = '<tr><td colspan="' + cols + '" style="color:#a1a1aa">' +
        (wholesaleMode ? "No wholesale SKUs in this category (50 lb and up only)." : "No products in this view.") +
        "</td></tr>";
      return;
    }
    var html = "";
    list.forEach(function (it) {
      ...
      var dealer = it.dealerPrice != null ? Number(it.dealerPrice) : Math.round((Number(it.price) || 0) * 0.85 * 100) / 100;
      var lb = it.weightLb != null ? String(it.weightLb) : "—";
      html += '<tr data-sku="' + esc(it.sku) + '" class="' + (listed ? "" : "row-hidden") + '">' +
        ...
        "<td>$" + retail + "</td>" +
        (wholesaleMode ? ("<td class=\"wh-only\">$" + Number(dealer).toFixed(2) + "</td><td class=\"wh-only\">" + esc(lb) + "</td>") : "") +
  ...
  function setInventoryView(view) {
    ...
    document.body.classList.toggle("master-mode", view === "master");
    document.body.classList.toggle("main-mode", view === "main");
    if ($("btnMain")) $("btnMain").classList.toggle("active", view === "main");
    if ($("btnWholesale")) {
      $("btnWholesale").classList.toggle("active", view === "wholesale");
      $("btnWholesale").textContent = "Wholesale";
    }
    if ($("btnMaster")) {
      $("btnMaster").classList.toggle("active", view === "master");
      $("btnMaster").textContent = "Master";
    }
    if ($("viewHint")) {
      if (view === "wholesale") {
        $("viewHint").textContent = "Wholesale";
        $("viewHint").style.color = "#fbbf24";
      } else if (view === "master") {
        $("viewHint").textContent = "Master list";
        $("viewHint").style.color = "#38bdf8";
      } else {
        $("viewHint").textContent = "Sales inventory";
        $("viewHint").style.color = "";
      }
    }
    filterCat = "";
    if (typeof renderChips === "function") renderChips();
    if (typeof renderTable === "function") renderTable();
    setStatus(view === "wholesale" ? "Wholesale" : view === "master" ? "Master list" : "Sales inventory", "ok");
  }

  if ($("btnMain")) $("btnMain").onclick = function () { setInventoryView("main"); };
  if ($("btnWholesale")) $("btnWholesale").onclick = function () { setInventoryView("wholesale"); };
  if ($("btnMaster")) $("btnMaster").onclick = function () { setInventoryView("master"); };
